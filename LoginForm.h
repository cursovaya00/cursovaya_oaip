...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button2->Text = L"Зарегестрироваться";
...
#pragma endregion
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {//войти
		try {
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {//зарегаться
		try {
			String^ user_login = textBox1->Text->Trim();
			String^ user_password = textBox2->Text->Trim();
			if (user_login == "" || user_password == "") {
			...
			if (us_reg) {
				MessageBox::Show("Теперь войдите в систему.", "Вы успешно зарегестрированы.", MessageBoxButtons::OK, MessageBoxIcon::None);
				this->textBox1->Clear();
				this->textBox2->Clear();
			}
			else { MessageBox::Show("Пользователь уже существует", "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error); }
		}
		catch (Exception^ ex) {
			MessageBox::Show(ex->Message, "Ошибка", MessageBoxButtons::OK, MessageBoxIcon::Error);
		}
